...
class CommentBody(db.Model):
    ...
    def inset(self, act_user, text_code, act_time, comment_body):
        a = CommentBody(actUser=act_user,textCode=text_code,actTime=act_time, commentBody= comment_body)
        db.session.add(a)
        print '插入数据中'
        db.session.commit()
        return 'yes'
